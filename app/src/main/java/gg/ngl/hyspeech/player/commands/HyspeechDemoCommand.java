...
public class HyspeechDemoCommand extends AbstractPlayerCommand {
    public HyspeechDemoCommand() {
        super("hytest", "Command for developer use only!");
    }

    @Override
    protected void execute(@Nonnull CommandContext commandContext, @Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref, @Nonnull PlayerRef playerRef, @Nonnull World world) {
        DemoClass testClass = new DemoClass();
        testClass.setCustomData("lol, what?");

        Hyspeech.get().registerContextContributor(ctx -> {
            ctx.put(DemoClass.class, testClass);
        });
    }
}
